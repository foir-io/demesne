...
The TypeScript half of Demesne's generated Layer-2 (read glue) and Layer-3 (control-plane
...
ready-to-call module: `mint`/`sessionSetupSQL`, a per-object `canView`/`canEdit` plus
...
- `packages/runtime`: `@demesne/runtime`, zero runtime dependencies. The algorithms:
  claims/session minting, the verb PDP and `composeCan`, the holds-resolver
  (`scopeContains` / `resolve`) and vocabulary expansion, the delegation cap, the
