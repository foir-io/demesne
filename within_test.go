...
object item {
  ...
  permission view = tenant->owner
                  ...
                  + (mode shared = "true" and @within(project nullable))
                  @rls maps select
  permission remove = (project->owner and @within(project)) @rls maps delete
}
`

func TestWithin_PerBranchScope(t *testing.T) {
	...
	sel := findPolicy(rls, "items_select")
	if sel == nil {
		t.Fatalf("no items_select (unsupported: %v)", rls.Unsupported)
	}
	u := sel.Using
	...
	if !strings.Contains(u, "auth.is_tenant_admin("+sub+", tenant_id)") {
		t.Errorf("missing 2-arg is_tenant_admin (root-level walk, unchanged):\n%s", u)
	}
	if !strings.Contains(u, "auth.is_project_admin("+sub+", tenant_id, project_id)") {
		t.Errorf("missing 3-arg is_project_admin (deeper-level walk arity):\n%s", u)
	}
	...
	if !strings.Contains(u, "(current_setting('request.jwt.claims', true)::json ->> 'kind') = 'service'") {
		t.Errorf("missing @kind term:\n%s", u)
	}
	if !strings.Contains(u, "shared = 'true'") {
		t.Errorf("missing mode term:\n%s", u)
	}
